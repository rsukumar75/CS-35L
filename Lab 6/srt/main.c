...
static void* img_split(void* argument)
{
  Vec3 camera_pos;    
  Vec3 camera_dir;
  ...
  struct tdetails* threadinfo = (struct tdetails*) argument;
  scene_t* scene = threadinfo->scene;
  float* colors = threadinfo->colors;
  /* for every pixel */
  for( int px=0, pos = 0, row_size = height * 3; px<width; ++px )
    {
      if(pos % threadinfo->numthreads != threadinfo->tid)
	{
	  pos += row_size;
	  continue;
	}
	  ...
	  for( int i=0; i<3; i++)
	    colors[pos + i] = max( min(scaled_color[i], 255), 0);
	}
    }
  return NULL;
}
...
int main( int argc, char **argv )
{
    ...
    float* colors = (float *) malloc(size * sizeof(float));
    struct tdetails* tlist = (struct tdetails*) malloc(nthreads * sizeof(struct tdetails));
	...
	tlist[k].tid = k;
	tlist[k].scene = &scene;
	tlist[k].numthreads = nthreads;
	tlist[k].colors = colors;
      }
    int i;
    for(i=0;i<nthreads;i++)
      {
	int ret = pthread_create(&threads[i], NULL, img_split, (void *) &tlist[i]);
	if(ret)
	  {
    ...
    while(pos+2 < size)
      {
	printf( "%.0f %.0f %.0f\n",
		colors[pos], colors[pos + 1], colors[pos + 2] );
	pos += 3;
	if(pos % (height*3) == 0)
	  {
	    printf("\n");
